...
$query_data ="
 SELECT Channel ,Region ,Country ,Trademark ,DATE_FORMAT(Date, '%m/%d/%Y') AS Date,Value FROM MEDIA_I WHERE Channel IN (".$search_text.") AND Country IN (".$search_text_market.") AND Trademark IN (".$search_text_trademark.") AND Value <> 0";
    ...
    $query_data ="
 SELECT Channel ,Region ,Country ,Trademark ,Year(Date) AS Date, sum(Value) as Value FROM MEDIA_I WHERE Channel IN (".$search_text.") AND Country IN (".$search_text_market.") AND Trademark IN (".$search_text_trademark.") AND Value <> 0 
