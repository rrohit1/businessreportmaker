...
$data = array();
   ...
   $query = "
    SELECT * FROM second_level_category_2 WHERE Market IN ('".$id."') group by Market
   ";
...
$query_data ="
 SELECT Channel ,Region ,Country ,Trademark ,DATE_FORMAT(Date, '%m/%d/%Y') AS Date,Value FROM MEDIA_I WHERE Channel IN (".$search_text.") AND Country IN (".$search_text_market.") AND Value <> 0";
}
else
{
